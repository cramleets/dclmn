...
function isGutenbergEditor() {
  return (
    typeof wp !== "undefined" && wp.data && wp.data.select("core/block-editor")
  );
}
...
jQuery(document).ready(function ($) {
  ...
  $(".show_hide_filter").on("click", function (e) {
    ...
    $(".eventful__filter_bar").toggleClass("active");

    // Toggle button text
    const isActive = $(".eventful__filter_bar").hasClass("active");
    ...
    if ($(".ta-container").length > 0) {
      $(".ta-container").each(function () {
        var eventful_container = $(this),
          ...
          layout_preset = $(eventful_Wrapper_ID).data("layout"),
          eventfulCarousel = $(
            ...
            if (
              ...
              eventfulSwiper = new Swiper(
                "#" + eventful_container_id + " .ta-eventful-carousel",
                {
                  ...
                  loop: eventfulCarouselData.loop,
        ...
        function eventful_pagination_action(selected_term_list = null) {
          ...
          jQuery.ajax({
            ...
            type: "POST",
            data: {
              action: "event_pagination_bar_mobile",
              id: eventful_sid,
              lang: ta_eventful_lang,
              keyword: keyword,
              taxonomy: taxonomy,
...
// independently with its own image-load + resize debounce so multiple
// shortcodes on the same page can't interfere with each other.
jQuery(function ($) {
  "use strict";
  ...
  same_height_item();
  // Re-equalize once the full page (fonts, late images) is settled.
...
(function () {
  ...
  var timelines = []; // array of { el, container } — container is null for front-end
  var ticking = false;
  ...
  function computeProgress(el, container) {
    ...
    // Start filling once the top of the timeline crosses 75% down the viewport;
    // finish once the bottom passes 25% down. The 50%-of-viewport "active band"
    // tracks the user's reading focus rather than the raw scroll position.
  ...
  function init() {
    ...
    // Attach scroll listener to each unique preview-box container found,
    // plus window for any front-end (non-preview) timelines.
    var attached = [];
    for (var i = 0; i < timelines.length; i++) {
    ...
    // Re-measure after images/fonts load — the timeline height grows.
...
// based on the underlying Swiper instance's `progress` (0..1). The CSS rule in
// _timeline.scss reads that var as the width of the ::after fill.
//
// Swiper attaches itself to the container element asynchronously (the carousel
// init IIFE earlier in this file runs on document ready). We poll briefly for
// `container.swiper` to appear, then subscribe to its `progress`/`slideChange`
...
(function () {
  ...
  function attach(container) {
    var tries = 0;
    var maxTries = 40; // ~2s at 50ms intervals
    (function poll() {
