 ...
 *
 */
if (!class_exists('EventfulMetabox')) {
  class EventfulMetabox extends EventfulAbstract
  {
    ...
    public function add_meta_box_content($post, $callback)
    {
      ...
      echo '<div class="eventful-framework eventful-metabox' . esc_attr($theme) . '">';
      ...
      if ($has_nav) {
        ...
        foreach ($this->sections as $section) {
          ...
          echo '<li class="menu-item_' . esc_attr($this->unique) . '_' . esc_attr($tab_key) . '"><a href="#" data-section="' . esc_attr($this->unique) . '_' . esc_attr($tab_key) . '">' . wp_kses_post($tab_icon) . wp_kses_post($section['title']) . wp_kses_post($tab_error) . '</a></li>';
      ...
      foreach ($this->sections as $section) {
        ...
        echo ($section_title || $section_icon) ? '<div class="eventful-section-title"><h3>' . wp_kses_post($section_icon) . wp_kses_post($section_title) . '</h3></div>' : '';
        ...
        if (!empty($section['fields'])) {
          ...
          foreach ($section['fields'] as $field) {
            ...
            Eventful::field($field, $this->get_meta_value($field), $this->unique, 'metabox');
          }
        } else {
      ...
      echo '</div>';
      echo '<button class="eventful_preview_btn" id="eventful-show-preview" data-id="' . esc_attr($post->ID) . '"href=""> <i class="icofont-eye-open" aria-hidden="true"></i>' . esc_html__('Show Preview', 'eventful')  . '</button>';
      if (!empty($this->args['show_restore']) || !empty($this->args['show_reset'])) {
        ...
        echo '<span class="button eventful-button-cancel">' . sprintf('<small>( %s )</small> %s', esc_html__('update event', 'eventful'), esc_html__('Cancel', 'eventful')) . '</span>';
        echo '</label>';
        echo '</div>';
      }
