 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'EventfulField_column' ) ) {

	/**
	 * The column field class.
	 */
	class EventfulField_column extends EventfulFields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				$this->field,
				array(
					'lg_desktop_icon'        => '<i class="icofont-laptop-alt"></i>',
			...
			if ( ! empty( $args['unit'] ) && count( $args['units'] ) > 1 ) {
				...
				foreach ( $args['units'] as $unit ) {
					...
					echo '<option value="' . esc_attr($unit) . '"' . esc_attr($selected) . '>' . wp_kses_post($unit) . '</option>';
				}
				echo '</select>';
				echo '</div>';
			}
