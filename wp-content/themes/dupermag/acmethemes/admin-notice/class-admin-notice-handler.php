...
class DuperMag_Notice_Handler {
	...
	public function replace_post_ids( $replace_post_ids ){
		...
		/*Post IDS*/
		$post_ids = array(
            'dupermag-feature-post-one',
            'dupermag-feature-post-two',
		);
	...
	public function replace_term_ids( $replace_term_ids ){
		...
		$term_ids = array(
            ...
            'dupermag-feature-cat',
            'dupermag-feature-side-from-category',
		);
	...
	public function admin_notice() {
		...
		$notice_nag = get_option( 'dupermag_admin_notice_welcome' );
		if ( ! $notice_nag ) {
			wp_enqueue_style( 'dupermag-notice', get_stylesheet_directory_uri() . '/acmethemes/admin-notice/admin-notice.css', array(), '3.0.0' );
			wp_enqueue_script( 'dupermag-adi-install', get_stylesheet_directory_uri()  . '/acmethemes/admin-notice/admin-notice.js', array( 'jquery' ), '', true );

			$translation = array(
                'btn_text' => esc_html__( 'Processing...', 'dupermag' ),
                'nonce'    => wp_create_nonce( 'dupermag_demo_import_nonce' ),
                'adminurl'    => admin_url()
            );
			wp_localize_script( 'dupermag-adi-install', 'dupermag_adi_install', $translation );
	...
	public static function hide_notices() {
		...
		if ( isset( $_GET['at-gsm-hide-notice'] ) && isset( $_GET['at_gsm_admin_notice_nonce'] ) ) { // WPCS: input var ok.
			if ( ! wp_verify_nonce( wp_unslash( $_GET['at_gsm_admin_notice_nonce'] ), 'at_gsm_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.VIP.ValidatedSanitizedInput.InputNotSanitized
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'dupermag' ) ); // WPCS: xss ok.
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'dupermag' ) ); // WPCS: xss ok.
			}

			$notice_type = sanitize_text_field( wp_unslash( $_GET['at-gsm-hide-notice'] ) );
			update_option( 'dupermag_admin_notice_' . $notice_type, 1 );

			/*Update to Hide.*/
			if ( 'welcome' === $_GET['at-gsm-hide-notice'] ) {
				update_option( 'dupermag_admin_notice_' . $notice_type, 1 );
			} else { // Show.
				delete_option( 'dupermag_admin_notice_' . $notice_type );
			}
		}
	...
	public function getting_started() {
        ...
        <div id="at-gsm" class="updated notice-info at-gsm">
            <a class="at-gsm-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'activated' ), add_query_arg( 'at-gsm-hide-notice', 'welcome' ) ), 'at_gsm_hide_notices_nonce', 'at_gsm_admin_notice_nonce' ) ); ?>">
				<?php esc_html_e( 'Dismiss', 'dupermag' ); ?>
            </a>
            <div class="at-gsm-container">
                <img class="at-gsm-screenshot" src="<?php echo esc_url(get_stylesheet_directory_uri().'/screenshot.jpg' )?>" alt="<?php esc_attr_e( 'DuperMag', 'dupermag' ); ?>" />
                <div class="at-gsm-notice">
                    <h2>
						<?php
						printf(
						/* translators: 1: welcome page link starting html tag, 2: welcome page link ending html tag. */
							esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer, please make sure you visit our %2$swelcome page%3$s.', 'dupermag' ), '<strong>'. wp_get_theme()->get('Name'). '</strong>','<a href="' . esc_url( admin_url( 'themes.php?page=dupermag-info' ) ) . '">','</a>' );
						?>
                    </h2>

                    <p class="plugin-install-notice"><?php esc_html_e( 'Clicking the button below will install and activate the Acme Demo Setup and Advanced Import plugins.', 'dupermag' ); ?></p>

                    <a class="at-gsm-btn button button-primary button-hero" href="#" data-name="" data-slug="" aria-label="<?php esc_attr_e( 'Get started with DuperMag', 'dupermag' ); ?>">
		                <?php esc_html_e( 'Get started with DuperMag', 'dupermag' );?>
                    </a>
                </div>
            </div>
        </div>
	...
	public function install_advanced_import() {
		...
		check_ajax_referer( 'dupermag_demo_import_nonce', 'security' );
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			$status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'dupermag' );
		...
		if ( is_wp_error( $result ) ) {
			...
			$status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'dupermag' );
	...
	public function setup_review_notice() {
		...
		// Set the installed time in `dupermag_theme_installed_time` option table.
		$option = get_option('dupermag_theme_installed_time' );

		if ( ! $option ) {
			update_option('dupermag_theme_installed_time', time() );
		}
	...
	public function display_review_notice() {
		 ...
		 */
		if ( ( get_option('dupermag_theme_installed_time' ) > strtotime( '-15 day' ) ) || ( $ignored_notice_partially > strtotime( '-15 day' ) ) || ( $ignored_notice ) ) {
			return;
		}
		?>
		<div class="notice updated at-review-notice">
			<p>
				...
				/* Translators: %1$s current user display name. */
					esc_html__(
						'Howdy, %1$s! It seems that you have been using this theme for more than 15 days. We hope you are happy with everything that the theme has to offer. If you can spare a minute, please help us by leaving a 5-star review on WordPress.org.  By spreading the love, we can continue to develop new amazing features in the future, for free!', 'dupermag'
					),
			...
			<div class="links">
				<a href="https://wordpress.org/support/theme/dupermag/reviews/?filter=5#new-post" class="btn button-primary" target="_blank">
					<span class="dashicons dashicons-thumbs-up"></span>
					<span><?php esc_html_e( 'Sure', 'dupermag' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice_partially=0" class="btn at-danger-btn">
					<span class="dashicons dashicons-calendar"></span>
					<span><?php esc_html_e( 'Maybe later', 'dupermag' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice=0" class="btn at-success-btn">
					<span class="dashicons dashicons-smiley"></span>
					<span><?php esc_html_e( 'I already did', 'dupermag' ); ?></span>
				</a>

				<a href="<?php echo esc_url( 'https://wordpress.org/support/theme/dupermag/' ); ?>" class="btn at-default-btn" target="_blank">
					<span class="dashicons dashicons-edit"></span>
					<span><?php esc_html_e( 'Got theme support question?', 'dupermag' ); ?></span>
				</a>
			</div>
	...
	public function remove_review_notice() {
		...
		global $current_user;
		$user_id                  = $current_user->ID;
		$theme_installed_time     = get_option('dupermag_theme_installed_time' );
		$ignored_notice           = get_user_meta( $user_id, 'remove_theme_review_notice', true );
		$ignored_notice_partially = get_user_meta( $user_id, 'nag_remove_theme_review_notice_partially', true );

		// Delete options data.
		if ( $theme_installed_time ) {
			delete_option('dupermag_theme_installed_time' );
		}
	...
	public function review_notice_enqueue() {
		...
		wp_enqueue_style( 'dupermag-review-notice', get_stylesheet_directory_uri()  . '/acmethemes/admin-notice/admin-notice.css' );
 ...
 * @since    1.0.0
 */
function dupermag_notice_handler( ) {
	return DuperMag_Notice_Handler::instance();
}
dupermag_notice_handler()->run();


/*For this theme only*/
if( !function_exists( 'dupermag_ai_update_image_size') ){
	function dupermag_ai_update_image_size(){
		/*Thumbnail Size*/
...
add_action( 'advanced_import_before_content_screen', 'dupermag_ai_update_image_size' );